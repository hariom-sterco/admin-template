...
const SectionBuilder = () => {
  const { props } = usePage();
  const { activeSections = [], sections: availableSections = [], page, images, flash, sectionRoutes, backUrl } = props;

  const getSectionStoreRoute = () => {
    if (sectionRoutes?.store) return sectionRoutes.store;
    return route('pages.sections.store', { page: page.id });
  };

  const getSectionDestroyRoute = (pivotId) => {
    if (sectionRoutes?.destroy) {
      return route(sectionRoutes.destroy, {
        ...(sectionRoutes.destroyParams || {}),
        pivot: pivotId,
      });
    }
    return route('pages.sections.destroy', pivotId);
  };
  ...
  const buildSectionDefaults = (selectedSection) => {
    const data = {};
    const mapping_items = {};

    (selectedSection.fields_config || []).forEach((fc) => {
      if (fc.type === 'checkbox') {
        data[fc.name] = fc.default ?? false;
      } else {
        data[fc.name] = fc.default ?? '';
      }
    });

    const groups = normalizeMappingConfig(selectedSection.mapping_config || []);
    groups.forEach((group) => {
      const item = {};
      (group.fields || []).forEach((fc) => {
        if (fc.type === 'checkbox') {
          item[fc.name] = fc.default ?? false;
        } else {
          item[fc.name] = fc.default ?? '';
        }
      });
      if (!mapping_items[group.group_name]) {
        mapping_items[group.group_name] = [];
      }
      mapping_items[group.group_name].push(item);
    });

    return { data, mapping_items };
  };
  ...
  const addSection = (sectionId) => {
    ...
    const defaults = buildSectionDefaults(selectedSection);
    const newSection = {
      section_id: parseInt(sectionId),
      order: sections.length,
      data: defaults.data,
      mapping_items: defaults.mapping_items,
      isCollapsed: false,
      files: {},
      mapping_files: {},
      isNew: true,
  ...
  const confirmDelete = () => {
    ...
    if (section.pivot_id) {
      router.get(getSectionDestroyRoute(section.pivot_id), {
        preserveScroll: true,
        preserveState: true,
        onSuccess: () => {
        ...
        onError: () => toast.error("Error deleting section"),
  ...
  /** @param {null|{ groupName: string, itemIndex: number, nestedGroup?: string|null, nestedItemIndex?: number|null }} mapCtx */
  ...
  /** When isMapping, `mapCtx` is { groupName, itemIndex, nestedGroup?, nestedItemIndex? } */
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!page?.id) { toast.error("Page not found"); return; }
    const changedSections = sections.filter((section) => {
      if (!section.section_id) return false;
      if (section.isNew || section.dirty) return true;
      if (section.files && Object.keys(section.files).length > 0) return true;
      if (section.mapping_files && Object.keys(section.mapping_files).length > 0) return true;
      if (!section.originalFingerprint) return true;
      return section.originalFingerprint !== createSectionFingerprint(section);
    });
    const hasOrderChange = sections.some((s) => {
      if (!s.originalFingerprint || !s.section_id) return false;
      try {
        const fp = JSON.parse(s.originalFingerprint);
        return (fp.order ?? 0) !== (s.order ?? 0);
      } catch {
        return false;
      }
    });
    const sectionsToSave = hasOrderChange
      ? sections.filter((s) => s.section_id)
      : changedSections;
    ...
    sectionsToSave.forEach((section, index) => {
      ...
      if (section.pivot_id) formData.append(`sections[${index}][pivot_id]`, section.pivot_id);
      ...
      if (section.files) Object.entries(section.files).forEach(([fn, file]) => { if (isFileObject(file)) formData.append(`sections[${index}][files][${fn}]`, file); });
        ...
        const appendMappingFileLeaves = (branch, pathParts) => {
          ...
          Object.entries(branch).forEach(([k, val]) => {
            if (isFileObject(val)) {
              ...
              val.forEach((entry, i) => {
                if (entry && typeof entry === 'object') appendMappingFileLeaves(entry, [...pathParts, k, String(i)]);
              });
    ...
    router.post(getSectionStoreRoute(), formData, {
      ...
      onError: (errors) => { setIsSaving(false); toast.error("Error saving: " + (errors.message || 'Unknown error')); }
    });
  };
          ...
          <div>
            {backUrl && (
              <Link href={backUrl} className="btn btn-sm btn-outline-secondary mb-2">
                ← Back to entry detail
              </Link>
            )}
            <h1 className="text-muted mb-1">{page?.title || 'Untitled Page'}</h1>
            <p className="text-muted mb-0 flex items-center">
              Slug:&nbsp;
              <span>{page?.slug || 'untitled-page'}</span>
            </p>
          </div>
