...
class ModuleEntry extends Model
{
    ...
    public function scopeForModule($query, int $id, string $sortField = 'date', string $direction = 'desc', bool $isJson = true)
    {
        ...
        if ($isJson) {
            $castType = $sortField === 'date' ? 'DATE' : 'UNSIGNED';
    ...
    private static function renderSectionHtmlTemplate(string $sectionHtml, array $sectionData): string
    {
        ...
        $flattened = self::flattenSectionData($data);
    ...
    private static function flattenSectionData(array $data, string $prefix = ''): array
    {
        $out = [];
        foreach ($data as $key => $value) {
            $currentKey = $prefix === '' ? $key : $prefix . '.' . $key;
            if (is_array($value) && self::isAssocArray($value)) {
                $out = array_merge($out, self::flattenSectionData($value, $currentKey));
                continue;
            }
            if (!is_array($value)) {
