...
class Page extends Model
{
    ...
    private function fieldsForTemplateReplacement(array $sectionData): array
    {
        ...
        if (isset($sectionData['data']) && is_array($sectionData['data'])) {
            $this->flattenSectionData($sectionData['data'], '', $out);
        }
        ...
        foreach ($sectionData as $key => $value) {
            ...
            if (is_array($value)) {
                $this->flattenSectionData($value, $key, $out);
                continue;
            }
    ...
    private function flattenSectionData(array $data, string $prefix, array &$out): void
    {
        foreach ($data as $key => $value) {
            $currentKey = $prefix === '' ? $key : $prefix . '.' . $key;
            if (is_array($value)) {
                if ($this->isAssocArray($value)) {
                    $this->flattenSectionData($value, $currentKey, $out);
                }
                continue;
            }
            if (!array_key_exists($currentKey, $out)) {
